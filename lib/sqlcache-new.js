...
function fillTables(cache, contract, tables, initSeq, cb) {
	...
	async.forEachOfSeries(tables, function(table, tabName, callback){
		...
		function secondTranslate(cb) {
			...
			if (tks && initSeq[key2].translate){
				async.eachOfLimit(kpairs, parallelLimit, function(keys, index, callback){
				//If the first key has a translate function specified
					contract[initSeq[key2].translate].call(keys[0], keys[1], function(err, data){
						if(err) return callback(err);
						kpairs[index][1] = data.values.keyValue;
		...
		if (initSeq[key1].translate){
			async.eachOfLimit(kpairs, parallelLimit, function(keys, index, callback2){
				//If the first key has a translate function specified
				contract[initSeq[key1].translate].call(keys[0], function(err, data){
					if(err) callback2(err);
					kpairs[index][0] = data.values.keyValue;
				})
			}, function(err){
				if (err) throw err;
	...
	async.forEachOf(initCalls, function(keyarray, call, callback){
		contract[call].call(function(err, data){
			...
			for (var i = 0; i < keyarray.length; i++) {
				...
				try{
					initSeq[key].max = initSeq[key].min + parseInt(data[initSeq[key].field]);
				} catch (err){
					return callback(err);
				}
			};
		...
		async.forEachOf(secCalls, function(keysobj, call, callback){
			...
			fillTables(contract, tables, initSeq, cb)
		})
	})
}
...
//This is a very expensive operation. should only be used if you have nothing better to do
sqlcache.prototype.initTables = function(contractName, cb){
	// Use the InitCalls and SecCalls to find the index ranges
	// 
	var self = this;
...
function preprocess(contract, contractName, SD){
	...
	for (var key in SD.initSeq){
		...
		if (ind.translate && !contract[ind.translate]) throw new Error("The index \'" + key + "s\' index translate (call) function \'" + ind.translate + "\' does not appear in the contract.")

		// Check that the translate function has a return feild "keyValue"

		var funcDef = getFunc(contract, ind.translate)
		if (!funcDef.outputs.some(function(output){return (output.name == key)})) throw new Error("The initialization call " + ind.translate + " does not have required field key");

		//Check that the specified field is one of the outputs of the call.
		funcDef = getFunc(contract, len.call);
		if (!funcDefs[0].outputs.some(function(output){return (output.name == len.field)})) throw new Error("The initialization call " + len.call + " does not have required field " + ind.field);

		//Ensure the type of the init seq return fields are uints
		if (! /uint/i.test(funcDefs[0].outputs.filter(function(output){return (output.name == len.field)})[0].type)) throw new Error("Initialization sequence calls are required to have fields which are uints. The call: " + len.call + " is getting field: " + len.field + " which is of a non-uint type")
	...
	for (var key in neededKeys){
		if(!seenKeys[key]) throw new Error("The key: " + key + " is required for initialization but initialization method not provided for it");
	}
