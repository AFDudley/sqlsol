...
sqlcache.prototype.initialize = function(contractName, cb){
	...
	async.eachOfSeries(structDef.callOrder, function(levelCalls, level, callback){
		//Construct the indices pairs for dependancies(ipairs)
		var ipairs = [];
		//do a recursive? look up for each key in order. take into account dependent keys
...
sqlcache.prototype.update = function(contractName, name, keys, cb){
	...
	args.push(function(err, output){processReturn(err, output, cb)});
