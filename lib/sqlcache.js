...
function preprocess(contract, contractName, SD){
	...
	for (tabName in SD.tables){
		...
		if (!table.keys) {
			...
			table.keys = [];
			funcDef = getFunc(contract, table.call);
		...
		for (var i = 0; i < table.keys.length; i++) {
			...
			inputs[i].name = table.keys[i];
			// Determine if this input needs to be quoted
			inputs[i] = processField(inputs[i])

			// Store for checking they are matched
			neededKeys[table.keys[i]] = true;
		};

		NSD.tables[tabName].inputs = inputs;
	...
	for (var key in SD.initSeq){
		...
		if (ind.deserialize) {
			if (!contract[ind.deserialize]) {
				throw new Error("The index \'" + key + "s\' index deserialize (call) function \'" + ind.deserialize + "\' does not appear in the contract.")
			}
